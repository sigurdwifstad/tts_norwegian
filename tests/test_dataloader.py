...
def print_sample(dataset, idx, play_audio=False, dump_debug_audio=False):
    ...
    # Compute the pause+deadend-trimmed version whenever this is a part_3
    # sample with mid-sentence pauses or deadend cuts, so both playback and
    # debug dump reflect what the dataloader/training actually sees (not the
    # raw untrimmed clip).
    playback_clip = clip
    if "part_3" in raw["id"] and (raw.get("pauses") or raw.get("deadend_cuts")):
        trimmed = trim_pauses(
            clip.unsqueeze(0), sr, raw["start"], raw.get("pauses", []),
            hard_cuts=raw.get("deadend_cuts", []),
            max_pause_sec=dataset.max_pause_sec, fade_ms=dataset.pause_fade_ms,
        ).squeeze(0)
        print(
            f"Trimmed    : {clip.shape[0]/sr:.2f}s -> {trimmed.shape[0]/sr:.2f}s "
            f"(removed {(clip.shape[0]-trimmed.shape[0])/sr:.2f}s of pauses/deadends)"
        )
